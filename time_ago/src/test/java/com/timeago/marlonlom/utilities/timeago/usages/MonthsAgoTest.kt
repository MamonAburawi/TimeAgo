...
package com.timeago.marlonlom.utilities.timeago.usages

import com.timeago.marlonlom.utilities.timeago.DataBuilder.getExpectedMessage
import com.timeago.marlonlom.utilities.timeago.DataBuilder.newLocalBundle
import com.timeago.marlonlom.utilities.timeago.DataBuilder.newMessagesResource
import com.timeago.marlonlom.utilities.timeago.DataBuilder.randomLanguageRef
import com.timeago.marlonlom.utilities.timeago.DataBuilder.useTimeAgo
import timeago.TimeAgo.Periods.*
import timeago.TimeAgoMessages
...
class MonthsAgoTest {
    ...
    fun shouldShowPastDateTimeWithAlmostOneMonth() {
        ...
        val expected = getExpectedMessage(localBundle!!, ABOUT_A_MONTH_PAST.propertyKey)
    ...
    fun shouldShowFutureDateTimeWithAlmostOneMonth() {
        ...
        val results = useTimeAgo(calendar.timeInMillis, timeAgoMessages)
        val expected = getExpectedMessage(localBundle!!, ABOUT_A_MONTH_FUTURE.propertyKey)
        assertEquals(expected, results)
    }
    ...
    fun shouldShowPastDateTimeWithSixMonths() {
        ...
        val results = useTimeAgo(calendar.timeInMillis, timeAgoMessages)
        val expected = getExpectedMessage(localBundle!!, X_MONTHS_PAST.propertyKey, 6)
        assertEquals(expected, results)
    }
    ...
    fun shouldShowFutureDateTimeWithSixMonths() {
        val calendar = getCalendarInstance().apply { add(MONTH, 6) }
        val results = useTimeAgo(calendar.timeInMillis, timeAgoMessages)
        val expected = getExpectedMessage(localBundle!!, X_MONTHS_FUTURE.propertyKey, 6)
        assertEquals(expected, results)
    }
}
