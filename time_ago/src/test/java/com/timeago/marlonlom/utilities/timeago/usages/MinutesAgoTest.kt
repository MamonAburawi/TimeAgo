...
class MinutesAgoTest {
    ...
    fun shouldShowPastDateTimeWithNineMinutes() {
        ...
        val expected = getExpectedMessage(localBundle!!, X_MINUTES_PAST.propertyKey, 9)
    ...
    fun shouldShowPastDateTimeWithOneMinute() {
        ...
        val results = useTimeAgo(calendar.timeInMillis, timeAgoMessages)
        val expected = getExpectedMessage(localBundle!!, ONE_MINUTE_PAST.propertyKey)
        assertEquals(expected, results)
    }
    ...
    fun shouldShowFutureDateTimeWithNineMinutes() {
        ...
        val results = useTimeAgo(calendar.timeInMillis, timeAgoMessages)
        val expected = getExpectedMessage(localBundle!!, X_MINUTES_FUTURE.propertyKey, 9)
        assertEquals(expected, results)
    }
    ...
    fun shouldShowFutureDateTimeWithOneMinute() {
        ...
        val results = useTimeAgo(calendar.timeInMillis, timeAgoMessages)
        val expected = getExpectedMessage(localBundle!!, ONE_MINUTE_FUTURE.propertyKey)
        assertEquals(expected, results)
    }
